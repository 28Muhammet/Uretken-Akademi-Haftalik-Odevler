...
public class YunanAl : MonoBehaviour
{
    ...
    private void AtesEtmeMenzili()
    {
        ...
        {
            if (dusmanlar.gameObject.CompareTag("Türk"))
            {
    ...
    void AtesEt(GameObject hedef)
    {
        ...
        if (Physics.Raycast(point.transform.position, point.transform.forward, out hit, atesEtmeMenzil))
        {
            ...
            if (Time.time > atesEtmeSikligi)
            {
                ...
                TürkAl turkAlComponent = hit.transform.gameObject.GetComponent<TürkAl>();
                PlayerController playerControllerComponent = hit.transform.gameObject.GetComponent<PlayerController>();

                if (turkAlComponent != null)
                {
                    turkAlComponent.SaglikDurumu(darbeGücü);
                }

                if (playerControllerComponent != null)
                {
                    playerControllerComponent.SaglikDurumu(darbeGücü);
                }
